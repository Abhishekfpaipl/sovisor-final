    ...
    <div class="" style="padding-top: 70px; background-color:var(--bg-forth)">
      <div class="container mb-3 py-5">
        ...
        <select v-model="selectedCategory" class="form-select" id="categorySelect">
          ...
          <option v-for="category in categoriesList" :key="category.name" :value="category.name">{{ category.name }}</option>
        </select>
      </div>
    </div>
    <div class="container">
...
export default {
  data() {
    return {
      servicesList: [
        "Digital Marketing", " Website Development", "Software Development",],
      categoriesList: [
      ...
      this.servicesList.forEach(service => {
        this.placesList.forEach(place => {
          this.categoriesList.forEach(category => {
            const sentence = `We are offering ${service} to ${this.title} from ${category.name} in ${place}.`;
